...
function generate_paghiper_billet($invoice, $params)
{
    ...
    // Aplicamos as taxas do gateway sobre o
    if (paghiper_must_exempt_taxes_for_invoice(($is_pix ? 'paghiper_pix' : 'paghiper'), $invoice_id)) {
...
/**
 * Checks if an invoice matches the settings maximum_payment_amount and minimum_payment_amount.
 * The check is made on the balance of the invoice.
 *
 * @since 2.5.0
 *
 * @param string $gateway   'paghiper' for Billet or 'paghiper_pix' for Pix context.
 * @param string $invoiceId
 *
 * @return string|bool returns true in cae the invoice is elegible or a HTML with the errors.
 */
function paghiper_is_balance_elegible_for_payment($gateway, $invoiceId)
{
    $params = getGatewayVariables($gateway);
    $minimumPaymentAmount = (float) (str_replace(',', '.', $params['minimum_payment_amount']) ?? 0);
    $maximumPaymentAmount = (float) (str_replace(',', '.', $params['maximum_payment_amount']) ?? 0);
    $systemUrl = $params['systemurl'];

    $invoice = localAPI('GetInvoice', ['invoiceid' => $invoiceId], $params['admin']);
    $invoiceBalance = (float) $invoice['balance'];

    $errorText = '';

    if (($maximumPaymentAmount > 0 && $invoiceBalance > $maximumPaymentAmount)) {
        $maximumPaymentAmount = number_format($maximumPaymentAmount, 2, ',', '');

        $errorText = "Pagamentos com Pix estão disponíveis apenas para valores menores ou iguais a R$$maximumPaymentAmount";
    }

    if ($errorText === '' && $minimumPaymentAmount > 0 && $invoiceBalance < $minimumPaymentAmount) {
        $minimumPaymentAmount = number_format($minimumPaymentAmount, 2, ',', '');

        $errorText = "Pagamentos com Pix estão disponíveis apenas para valores a partir de R$$minimumPaymentAmount";
    }
    ...
    $assetFilename = $gateway === 'paghiper_pix' ? 'pix' : 'billet';

    return <<<HTML
    <img src='{$systemUrl}/modules/gateways/paghiper/assets/img/$assetFilename.jpg' title='Pagar com Pix' alt='Pagar com Pix' border='0' align='absbottom' width='120' height='74' /><br>
    <div style="margin-top: 5px" class='alert alert-warning' role='alert'>
        $errorText
    </div>
    HTML;
}
 ...
 * @return bool
 */
function paghiper_must_exempt_taxes_for_invoice($gateway, $invoiceId)
{
    $params = getGatewayVariables($gateway);
    $minimumPaymentAmountForExemption = (float) ($params['min_amount_for_taxes_exempt'] ?? 0);

    if ($minimumPaymentAmountForExemption === 0.0 || $minimumPaymentAmountForExemption === '') {
        return false;
    }
    ...
    return $invoiceBalance >= $minimumPaymentAmountForExemption;
}
