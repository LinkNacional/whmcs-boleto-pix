...
if (!defined('WHMCS')) {
    ...
    if (isset($_GET['invoiceid'])) {
        ...
        $isElegibleForPayment = paghiper_is_balance_elegible_for_payment($gateway_code, $invoice['invoiceid']);

        if (is_string($isElegibleForPayment)) {
            $ico = $is_pix ? 'pix-cancelled.png' : 'billet-cancelled.png';
            $title = 'Fatura não disponível para pagamento via ' . ($is_pix ? 'PIX' : 'boleto') . '.';
            $message = 'Fatura não disponível para pagamento via ' . ($is_pix ? 'PIX' : 'boleto') . '.';
        ...
        if (paghiper_must_exempt_taxes_for_invoice($gateway_code, $order_id)) {
            $invoice_total = number_format($invoice['balance'], 2, '.', '');
        } else {
        ...
        if (isset($reissue) && $reissue) {
            ...
            if (!empty($discount_period) && $discount_period > 0) {
                if ($vencimentoBoleto <= $discount_period || $vencimentoBoleto == 0) {
                    unset($GATEWAY['early_payment_discounts_days'], $GATEWAY['early_payment_discounts_cents']);
                }
            }
            ...
            if ($return_json) {
                ...
                if (!empty($qrcode_image_url)) {
                    echo paghiper_print_screen($qrcode_image_url, null, null, ['is_pix' => true, 'invoice_id' => $order_id, 'payment_value' => $billet_value, 'pix_emv' => $emv]);
                } else {
    ...
    // Caso contrário, é um post do PagHiper.
        ...
        if (!empty($_POST)) {
            ...
            if (!$lock_id || !$is_strong) {
                ...
                echo paghiper_print_screen($ico, $title, $message);
                logTransaction($gateway_settings['name'], ['invoice_id' => $invoice_id, 'exception' => 'Failed to write Paghiper LockID'], sprintf('Não foi possível associar o ID de requisição ao %s.', ($is_pix) ? 'PIX' : 'boleto'));
                exit();
            }
            ...
            if (!$current_lock_id || ($current_lock_id !== $request_id)) {
                ...
                echo paghiper_print_screen($ico, $title, $message);
                logTransaction($gateway_settings['name'], ['invoice_id' => $order_id, 'exception' => 'Thread ID error (0x004682)'], sprintf("O ID de requosição associado %s não é desta sessão. Erro 0x004682 \n\nThread ID: %s\nLock: %s", (($is_pix) ? 'PIX' : 'boleto'), $request_id, $current_lock_id));
                exit();
            } else {
        ...
        if ($request['result'] == 'reject') {
            ...
            logTransaction($GATEWAY['name'], ['request' => $paghiper_data, 'post' => $_POST, 'json' => $json], 'Notificação Inválida.');
            ...
            // Transação Cancelada.
            } elseif ($status == 'canceled' || $status == 'Cancelado') {
